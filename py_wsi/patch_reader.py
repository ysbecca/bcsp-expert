...
def get_regions(path):
	...
	try:
		...
		print("Attempting to read as JSON...")
		# Couldn't find XML; try reading JSON files.
...
def sample_and_store_patches_by_row(
	...
	slide = open_slide(file_dir + file_name) # .svs
	tiles = DeepZoomGenerator(slide, tile_size=tile_size, overlap=pixel_overlap, limit_bounds=limit_bounds)

	if xml_dir:
		# Expect filename of annotation to match SVS file name
		regions, region_labels = get_regions(xml_dir + file_name[:-4])
