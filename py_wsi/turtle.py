...
class Turtle(object):
	...
	# Customised for reading from folders by Case.
	def __init__(self, file_dir, db_location, db_name, xml_dir=False, label_map={}, cases=[]):
		self.file_dir = file_dir
		self.db_location = db_location
		self.db_name = db_name
		self.db_meta_name = self.__get_db_meta_name(db_name)
		self.files, self.cases = self.__get_files_from_dir(file_dir, cases)
	...
	def __get_files_from_dir(self, file_dir, cases, file_type='.svs'):
		if len(cases) == 0:
			...
			files = []
			for case in cases:
				case_file_dir = file_dir + str(case) + "/"
				case_files = np.array([file for file in listdir(case_file_dir) if isfile(join(case_file_dir, file)) and file_type in file])
				files = np.concatenate((
					case_files,
					np.array(files)), axis=0)
				cases_by_file = np.concatenate(([case]*len(case_files), cases_by_file), axis=0)

		return files, cases_by_file
	...
	def get_xml_files(self):
		files = self.__get_files_from_dir(self.xml_dir, '.xml')
		if len(files) == 0:
			files = self.__get_files_from_dir(self.xml_dir, '.json')
	...
	def calculate_map_size(self, patch_size, level, overlap, limit_bounds):
		...
		for i, file in enumerate(self.files):
			if len(self.cases) == 0:
				...
				slide = open_slide(self.file_dir + self.cases[i] + "/" + file)
	...
	def retrieve_tile_dimensions(self, file_name, patch_size=0, overlap=0, tile_size=0):
		...
		if len(self.cases) == 0:
			...
			slide = open_slide(self.file_dir + self.cases[i] + "/" + file_name)
